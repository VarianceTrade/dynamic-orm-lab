...
describe Student do
  ...
  context 'has instance methods to insert data into db' do
    ...
    describe '#save' do
      it 'saves the student to the db' do
        new_student.save
        expect(DB[:conn].execute("SELECT * FROM students WHERE name = 'Sam'")[0]).to include({"id"=>1, "name"=>"Sam", "grade"=>11})
      end
  ...
  describe '.find_by_name' do
    it 'executes the SQL to find a row by name' do
      ...
      expect(Student.find_by_name("Jan")[0]).to include({"id"=>1, "name"=>"Jan", "grade"=>10})
    end
  end
  ...
  describe '.find_by' do
    it 'executes the SQL to find a row by the attribute passed into the method' do
      ...
      expect(Student.find_by({name: "Susan"})[0]).to include({"id"=>1, "name"=>"Susan", "grade"=>10})
    end

    it 'accounts for when an attribute value is an integer' do
      Student.new({name: "Susan", grade: 10}).save
      Student.new({name: "Geraldine", grade: 9}).save
      expect(Student.find_by({grade: 10})[0]).to include({"id"=>1, "name"=>"Susan", "grade"=>10})
    end
  end
end
